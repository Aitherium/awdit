...
Rules:
  * **MOAT001** no monorepo import (`lib.`, `services.`, `from AitherOS`). awdit
...
archive) — never 0 for "I could not look".
    ...
    python scripts/check_moat_boundary.py [dist/awdit-*.whl dist/*.tar.gz ...]
...
_KEYSTONES = ("awdit/__init__.py", "awdit/cli.py", "awdit/log.py", "awdit/verify.py")
...
def self_test() -> int:
    ...
    def wheel(members: dict) -> Path:
        p = Path(tempfile.mkdtemp()) / "awdit-0.0.0-py3-none-any.whl"
    ...
    check("MOAT001 catches a monorepo import",
          any(f.startswith("MOAT001") for f in
              inspect(wheel({**clean, "awdit/x.py": "from lib.core import X\n"}))), True)

    check("MOAT002 catches a debt id",
          any("debt-ledger row id" in f for f in
              inspect(wheel({**clean, "awdit/x.py": "# see D-0000\n"}))), True)

    check("MOAT002 catches a checker rule id",
          any("checker rule id" in f for f in
              inspect(wheel({**clean, "awdit/x.py": "# MOAT001 says so\n"}))), True)
    ...
    try:
        broken = Path(tempfile.mkdtemp()) / "awdit-0.0.0.tar.gz"
